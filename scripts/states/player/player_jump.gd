...
var landing_circle: DrawCircle
var inner_landing_circle: DrawCircle
...
func enter(_previous_state_path: String = "", data := {}) -> void:
	...
	# Calculate jumping time
	# Let the player hang in the air based on jump_time
	# Have the player land and deal damage based on the area of the weapon

## Launch hte player into the air and play the jump animation.
...
## Jump up for a set amount of time based on [jump_speed]
func jump_up(delta: float) -> void:
	# Jump up for a set amount of time.
	jump_up_time += delta
	player.sprite_parent.position.y -= jump_speed * delta # Use -= because -y is up in Godot.

## Jump down for a set amount of time until we reach our original position.
func jump_down(delta: float) -> void:
