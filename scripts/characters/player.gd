...
var stats: Stats ## The stats object to hold the player character's vital statistics
...
## Initialize the player character based on [param character] that includes a [stats] object
func initialize(character: PlayerCharacterData) -> void:
	if stats: stats = null
	stats = Stats.new(character.stats)

	# Connects the signals from the stats object to the player.
	stats.damaged.connect(_on_player_damaged)
	stats.defeated.connect(_on_player_defeated)
	...
	# Emit the signals to update the UI
	Events.ui_update_hp.emit(stats.health, stats.max_health)
	Events.ui_update_xp.emit(experience, xp_to_next_level)

	# Don't forget to reset the UI!
	# TODO - restore player UI and health bar
	#adjust_health_bar()
...
func _physics_process(delta: float) -> void:
	...
	if not enemies_touching.is_empty():
		...
		if enemies_touching_last_frame != enemies_touching.size(): 
			...
			for e in enemies_touching: total_contact_damage += enemies_touching[e].contact_damage # Remember that the enemy dict is { ID, enemy } so we need to use the key
			enemies_touching_last_frame = enemies_touching.size()
		# Deal damage to the player.
		stats.apply_damage(total_contact_damage * delta)
...
func _on_player_collider_area_exited(area:Area2D) -> void:
	if area is BaseEnemy and enemies_touching.has(area.get_instance_id()): enemies_touching.erase(area.get_instance_id())
...
## Handles the player taking damage from enemies, called by the stats object.
...
func _on_player_defeated() -> void:
	state_machine.change_state(AnimationNames.DEATH)

## Handles the player gaining experience. If the player levels up, show the level up UI and emit the signals to update the UI.
func gain_experience(xp: float) -> void:
	# If the player gets enough XP, level up!
	experience += xp
	Events.player_gained_xp.emit(xp)
	Events.ui_update_xp.emit(experience, xp_to_next_level)
...
func level_up() -> void:
	var tween = show_level_up_text()
	await tween.finished # Wait for the tween to finish before continuing
	...
	# Emit the signals to update the UI
	Events.player_gained_level.emit()
	Events.ui_update_level.emit(level)
	Events.ui_update_xp.emit(experience, xp_to_next_level)

	# Display the Level Up UI from the Game Manager
	GameController.level_up_UI.show_level_up_screen() # TODO - decouple this from the game manager

## Function to animate the tween the level up text for each character's leveling phrase.
func show_level_up_text() -> Tween:
