...
var weapon: Weapon ## The weapon that spawned this projectile
var projectile_name: String
# Weapon-derived attributes
var damage: float
var damage_modifier: float = 1.0 ## The damage modifier for this projectile. Used for scaling damage with a variety of effects.
var damage_cooldown: float = 0.0 ## How often the projectile will deal damage to the target if applicable.
var speed: float ## Uses the projectile's own speed instead of the weapon speed as they may serve different functions.
...
## [b]Required:[/b] [code]spawning_weapon: Weapon, data: ProjectileData spawn position:Vector2, z-index: SpriteConstants.Z_INDEX[/code] [br]
...
func initialize(spawning_weapon: Weapon, data: ProjectileData, spawn_pos: Vector2, sprite_z_index: int, target: Node2D = null) -> void:
	...
	# ProjectileData attributes
	projectile_name = data.name
...
func _ready() -> void:
	name = projectile_name # Name assignment requires the node to be in the scene tree
	if lifetime > 0.0: enable_timer_expiration()

	# if projectile_name: print_debug("Loaded " + name)
	# else: print_debug("Loaded " + name + " which had no projectile name.")

## Deal damage to a target based on the damage modifier of the projectile and the damage of the weapon.[br]
