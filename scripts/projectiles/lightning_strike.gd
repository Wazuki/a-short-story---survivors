class_name LightningStrike extends Projectile
...
# func set_spawn_state(spawning_weapon: Weapon, spawn_pos: Vector2, strike_lifetime: float, dmg_cooldown: float) -> void:
...
# Add the enemy to the affected_bodies Dict so we can damage them on cooldown.
func _on_area_entered(area:Node2D) -> void: 
	affected_areas.set(area.get_instance_id(), area)

# Remove the enemy from the affected_bodies Dict when they leave the area
