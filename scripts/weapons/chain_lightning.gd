...
func _ready() -> void:
	...
	icon = preload("res://assets/UI/icons/chain_lightning_icon.tres")
	# reset()
...
func spawn_lightning_stike(spawn_pos: Vector2) -> void:
	...
	if LightningStrike.can_spawn_strike_at_pos(spawn_pos, get_tree()):
		...
		new_strike.initialize_strike(self, spawn_pos, lightning_strike_lifetime, lightning_strike_damage_cooldown)
