...
var character: PlayerCharacterData
var clickable = false
var selected = false
...
func initialize(c: PlayerCharacterData) -> void:
	character = c
	icon.texture = c.icon
	...
	#character.stats = c.stat_map.to_dict()
...
func _gui_input(event: InputEvent) -> void:
	if event is InputEventMouseButton and event.is_pressed() and clickable:
		...
		if not selected:
			selected = true
			# modulate = Color.GRAY
			material = preload("res://assets/shaders/scanline_material.tres")
